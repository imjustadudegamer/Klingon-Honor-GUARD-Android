	...
	if( Level->Model->Leaves.Num() )
		for( FDynamicSprite* Sprite = Frame->Sprite; Sprite; Sprite=Sprite->RenderNext )
			if( Sprite->Actor->Region.iLeaf!=INDEX_NONE
			 && Sprite->Actor->Region.iLeaf < Level->Model->Leaves.Num() )   // [KHG] guard: a stale Region.iLeaf would OOB-read LeafLights[] (sized Leaves.Num())
...
void URender::DrawFrame( FSceneNode* Frame )
{
	...
	&&	Frame->Recursion==0 )
	{
		...
		for( i=0; i<MAX_CORONA_LIGHTS; i++ )
		{
			...
			if( Light && CoronaLights[i].iActor<Viewport->Actor->XLevel->Num() && Viewport->Actor->XLevel->Actors(CoronaLights[i].iActor)==Light ) // [KHG] was <= : off-by-one OOB Actors() read
			{
				check(Light->IsValid());
