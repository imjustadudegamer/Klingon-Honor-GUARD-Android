...
// [KHG 219] CurrentFrameIndex + IMPLEMENT_CLASS live in VulkanDrv.cpp. Surface uses vkCreateAndroidSurfaceKHR
// (see Init), not SDL_Vulkan_*. Android native window header for the surface:
#include <android/native_window.h>
#include <SDL_syswm.h>

// [KHG 219] Instance defaults (was UT99 StaticConstructor's value block). URenderDevice base flags + config.
UVulkanRenderDevice::UVulkanRenderDevice()
{
	...
	UseVSync = 0;          // Android: present mode picked by swapchain; no vsync toggle
	AntialiasMode = 0;
	UsePrecache = 1;
	GammaMode = 0;
...
// [KHG 219] property registration (was inside StaticConstructor). 219 uses InternalClassInitializer(UClass*).
void UVulkanRenderDevice::InternalClassInitializer( UClass* Class )
{
	guard(UVulkanRenderDevice::InternalClassInitializer);
	new(Class, TEXT("UseVSync"), RF_Public) UBoolProperty(CPP_PROPERTY(UseVSync), "Options", CPF_Config);
	new(Class, TEXT("LinearBrightness"), RF_Public) UByteProperty(CPP_PROPERTY(LinearBrightness), "Options", CPF_Config);
	new(Class, TEXT("Contrast"), RF_Public) UByteProperty(CPP_PROPERTY(Contrast), "Options", CPF_Config);
	new(Class, TEXT("Saturation"), RF_Public) UByteProperty(CPP_PROPERTY(Saturation), "Options", CPF_Config);
	new(Class, TEXT("LODBias"), RF_Public) UFloatProperty(CPP_PROPERTY(LODBias), "Options", CPF_Config);
	new(Class, TEXT("VkDeviceIndex"), RF_Public) UIntProperty(CPP_PROPERTY(VkDeviceIndex), "Options", CPF_Config);
	new(Class, TEXT("VkDebug"), RF_Public) UBoolProperty(CPP_PROPERTY(VkDebug), "Options", CPF_Config);
	unguard;
}
