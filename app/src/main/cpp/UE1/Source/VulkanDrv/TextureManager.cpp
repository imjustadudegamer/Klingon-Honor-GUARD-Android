...
void TextureManager::UpdateTextureRect(FTextureInfo* info, int x, int y, int w, int h)
{
	...
	if (tex)
	{
		...
		info->TextureFlags &= ~TF_RealtimeChanged;   // [KHG 219] no bRealtimeChanged field; use TextureFlags
	}
}
...
#else
	else if (info->TextureFlags & TF_RealtimeChanged)   // [KHG 219] FMV/ScriptedTexture re-upload
	{
