...
struct FPropertyTag
{
	...
	,	Info		( Property->GetID() )
	{
		...
		:	Size==16	? 0x40
		:	Size<=255	? 0x50
		:	Size<=65535 ? 0x60   /* [KHG] was <=65536: 0x60 encodes Size as a 16-bit _WORD (max 65535); Size==65536 truncated to 0, desyncing the tagged-property stream */
		:			      0x70);
...
void UFunction::Bind()
{
	...
	if( FunctionFlags & FUNC_Intrinsic )
	{
		...
		else
		{
			...
			{
				debugf( NAME_Warning, "[KHG] Missing native function '%s' (%s); using execUndefined stub", Proc, GetPathName() );
				Func = &UObject::execUndefined;
			}
		}
	}
