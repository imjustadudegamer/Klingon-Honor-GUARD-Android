...
static UBOOL GAndroidFSReady = false;
static char GAndroidExternalFilesDir[1024] = "";
static char GAndroidUnrealRootDir[1024] = "";
static char GAndroidUnrealSystemDir[1024] = "";
static char GAndroidUnrealSaveDir[1024] = "";

static void AndroidEnsureTrailingSlash( char* Path, INT MaxLen )
{
	if( !Path || !Path[0] )
		return;

	INT Len = appStrlen( Path );
	if( Len > 0 && Path[Len - 1] != '/' && Len + 1 < MaxLen )
	{
		Path[Len] = '/';
		Path[Len + 1] = 0;
	}
...
static UBOOL AndroidDirExists( const char* Path )
{
	struct stat St;
	return Path && stat( Path, &St ) == 0 && S_ISDIR( St.st_mode );
}

static UBOOL AndroidFileExistsReadable( const char* Path )
{
	struct stat St;
	return Path && stat( Path, &St ) == 0 && S_ISREG( St.st_mode ) && access( Path, R_OK ) == 0;
}

static UBOOL AndroidDirHasAnyUnr( const char* Path )
{
	DIR* D = opendir( Path );
	if( !D )
		return false;
...
static void AndroidSetUnrealRoot( const char* Root )
{
	...
	appStrncpy( GAndroidUnrealRootDir, Root, sizeof(GAndroidUnrealRootDir) - 1 );
	GAndroidUnrealRootDir[sizeof(GAndroidUnrealRootDir) - 1] = 0;
	AndroidEnsureTrailingSlash( GAndroidUnrealRootDir, sizeof(GAndroidUnrealRootDir) );

	snprintf( GAndroidUnrealSystemDir, sizeof(GAndroidUnrealSystemDir), "%sSystem/", GAndroidUnrealRootDir );
	snprintf( GAndroidUnrealSaveDir,   sizeof(GAndroidUnrealSaveDir),   "%sSave/",   GAndroidUnrealRootDir );

	GAndroidUnrealSystemDir[sizeof(GAndroidUnrealSystemDir) - 1] = 0;
...
static void AndroidMakeDirsRecursive( const char* Path )
{
	...
	char Tmp[1024];
	appStrncpy( Tmp, Path, sizeof(Tmp) - 1 );
	Tmp[sizeof(Tmp) - 1] = 0;

	INT Len = appStrlen( Tmp );
	if( Len > 1 && Tmp[Len - 1] == '/' )
		Tmp[Len - 1] = 0;
	...
	for( char* P = Tmp + 1; *P; ++P )
	{
		...
		{
			*P = 0;
			if( Tmp[0] && !AndroidDirExists( Tmp ) )
				mkdir( Tmp, 0775 );
			*P = '/';
		}
	}

	if( !AndroidDirExists( Tmp ) )
	{
		if( mkdir( Tmp, 0775 ) != 0 && errno != EEXIST )
			UE1_ANDROID_LOGE( "mkdir failed: %s errno=%d %s", Tmp, errno, strerror(errno) );
	}
}

static void AndroidMakeParentDirForFile( const char* Filename )
{
	if( !Filename || !Filename[0] )
		return;

	char Dir[1024];
	appStrncpy( Dir, Filename, sizeof(Dir) - 1 );
	Dir[sizeof(Dir) - 1] = 0;
	...
	*Slash = 0;
	AndroidMakeDirsRecursive( Dir );
}

static const char* AndroidBaseName( const char* Filename )
{
...
static UBOOL AndroidLooksLikeIni( const char* Filename )
{
	if( !Filename || !Filename[0] )
		return false;

	const char* Dot = strrchr( AndroidBaseName(Filename), '.' );
...
CORE_API void appAndroidInitFileSystem()
{
	...
	// UNREAL_ANDROID_CONFIG_ROOT_FALLBACK_V16
	// Java can find /storage/emulated/0/Unreal correctly, but the native file
	// layer may be initialized before SDL_main argv/env propagation is visible.
	// Therefore the native filesystem layer performs the same data-root scan.
	const char* Base = SDL_AndroidGetExternalStoragePath();
	if( !Base || !Base[0] )
	{
		UE1_ANDROID_LOGW( "SDL_AndroidGetExternalStoragePath failed: %s", SDL_GetError() );
		Base = SDL_AndroidGetInternalStoragePath();
	}
	if( !Base || !Base[0] )
		Base = "/storage/emulated/0/Android/data/com.khg.android/files";
	...
	AndroidMakeDirsRecursive( GAndroidUnrealSaveDir );

	UE1_ANDROID_LOGI( "External files dir: %s", GAndroidExternalFilesDir );
	UE1_ANDROID_LOGI( "Unreal root dir:    %s", GAndroidUnrealRootDir );
	...
	GAndroidFSReady = true;
}
CORE_API const char* appAndroidExternalFilesDir()
{
	appAndroidInitFileSystem();
	return GAndroidExternalFilesDir;
}

CORE_API const char* appAndroidUnrealRootDir()
{
	appAndroidInitFileSystem();
	return GAndroidUnrealRootDir;
}

CORE_API const char* appAndroidUnrealSystemDir()
{
	appAndroidInitFileSystem();
	return GAndroidUnrealSystemDir;
}

CORE_API const char* appAndroidUnrealSaveDir()
{
	appAndroidInitFileSystem();
	return GAndroidUnrealSaveDir;
}
#endif
// UNREAL_ANDROID_CONFIG_PATHS_END
...
CORE_API FILE* appFopen( const char* Path, const char* Mode )
{
	...
	if( Path && Mode && Mode[0] != 'r' )
		AndroidMakeParentDirForFile( Path );
#endif
	...
	if( !F && Mode && Mode[0] == 'r' )
	{
		// Case-insensitive search for Linux/Android package files.
...
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	if( AndroidLooksLikeIni( Path ) )
	{
		if( F )
			UE1_ANDROID_LOGI( "fopen OK: mode=%s path=%s", Mode ? Mode : "?", Path ? Path : "(null)" );
		else
			UE1_ANDROID_LOGE( "fopen FAILED: mode=%s path=%s errno=%d %s", Mode ? Mode : "?", Path ? Path : "(null)", errno, strerror(errno) );
	}
#endif
...
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	if( AndroidLooksLikeIni( Filename ) )
		UE1_ANDROID_LOGI( "Config load path: %s", Filename ? Filename : "(null)" );
#endif
...
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	if( AndroidLooksLikeIni( Filename ) )
		UE1_ANDROID_LOGI( "Config loaded OK: %s bytes=%d", Filename ? Filename : "(null)", Size );
#endif
...
CORE_API UBOOL appSaveStringToFile( const FString& Str, const char* Filename )
{
...
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	if( AndroidLooksLikeIni( Filename ) )
		UE1_ANDROID_LOGI( "Config save path: %s", Filename ? Filename : "(null)" );
	...
	{
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
		UE1_ANDROID_LOGE( "Config save FAILED opening: %s errno=%d %s", Filename ? Filename : "(null)", errno, strerror(errno) );
#endif
		return false;
	}
	...
	UBOOL Ok = ( CharArray.Num() <= 1 || Wx == CharArray.Num() - 1 );

#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	if( AndroidLooksLikeIni( Filename ) )
	{
		if( Ok )
			UE1_ANDROID_LOGI( "Config saved OK: %s bytes=%d", Filename ? Filename : "(null)", Wx );
		else
			UE1_ANDROID_LOGE( "Config save FAILED writing: %s wrote=%d expected=%d", Filename ? Filename : "(null)", Wx, CharArray.Num() - 1 );
	}
#endif
...
TryAgain:
	snprintf( Filename, sizeof(Filename), "%s%s.%s", appBaseDir(), Package, LangExt );   // [KHG] bounded: appBaseDir() is a long Android external-storage path; was unbounded appSprintf into [256]
