...
/*-----------------------------------------------------------------------------
	UStrProperty.  (dynamic FString-backed string)

	Added to bind the 'StrProperty' native class present in build-219 Core.u
	(Star Trek: Klingon Honor Guard). See UnType.h for context.
-----------------------------------------------------------------------------*/
...
void UStrProperty::Serialize( FArchive& Ar )
{
	...
	UProperty::Serialize( Ar );
	// A dynamic string property carries no extra persistent fields beyond
	// UProperty (unlike UStringProperty, which serializes its fixed StringSize).
	unguard;
}
...
void UStrProperty::ExportTextItem( char* ValueStr, BYTE* PropertyValue, BYTE* DefaultValue, UBOOL HumanReadable )
{
	guard(UStrProperty::ExportTextItem);
	const char* Str = **(FString*)PropertyValue;
	if( HumanReadable )
		appStrcpy( ValueStr, Str );
	else
		appSprintf( ValueStr, "\"%s\"", Str );
	unguard;
}
const char* UStrProperty::ImportText( const char* Buffer, BYTE* Data, UBOOL HumanReadable ) const
{
	guard(UStrProperty::ImportText);
	if( HumanReadable )
	{
		*(FString*)Data = FString( Buffer );
	...
	else
	{
		...
		if( !Buffer )
			return NULL;
		*(FString*)Data = FString( Temp );
	}
	return Buffer;
	unguard;
}
void UStrProperty::ExecLet( void* Var, FFrame& Stack )
{
	guardSlow(UStrProperty::ExecLet);

	// Evaluate the right-hand side into a temporary string buffer, then assign
	// it into the destination FString.
	BYTE Buffer[MAX_STRING_CONST_SIZE], *Val=Buffer;
	Stack.Step( Stack.Object, Val );
	if( Var )
		*(FString*)Var = FString( (char*)Val );
