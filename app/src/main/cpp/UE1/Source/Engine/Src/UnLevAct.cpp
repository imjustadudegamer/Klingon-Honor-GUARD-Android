...
)
{
	...
	for( Test = FirstHit; Test; Test=Test->GetNext() )
		if
		(	Test->Actor                  // [KHG] guard: a prior iteration's eventEncroachedBy/BeginTouch script can DestroyActor a later actor in this hit list (mover/lift crush) -> use-after-free
		&&	!Test->Actor->bDeleteMe
		&&	Test->Actor!=Actor
		&&	Test->Actor!=GetLevelInfo() )
		{
			if( Actor->IsBlockedBy(Test->Actor) )
...
void ULevel::SetActorZone( AActor* Actor, UBOOL bTest, UBOOL bForceRefresh )
{
	...
	if( NewRegion.Zone!=Actor->Region.Zone )
	{
		...
		if( !bTest )
		{
			if( Actor->Region.Zone )   // [KHG] guard: actor in unzoned space (Region.Zone==NULL) would null-deref; engine only debug-asserts this (compiled out in this build)
		...
		if( !bTest )
		{
			if( Actor->Region.Zone )   // [KHG] guard: NewRegion.Zone may be NULL (unzoned space)
