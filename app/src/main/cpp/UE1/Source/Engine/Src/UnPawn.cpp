...
void AActor::CheckNoiseHearing(FLOAT Loudness)
{
	...
	if ( !NoiseOwner->bIsPlayer )
	{
		...
		if ( !NoiseOwner || !NoiseOwner->bIsPlayer )
		{
			...
			while ( next )
			{
				APawn* following = next->nextPawn;   // [KHG] cache the link BEFORE the event: eventHearNoise is script that can DestroyActor a PawnList pawn, freeing `next`
...
void APawn::ShowSelf()
{
	...
	while ( Pawn )
	{
		APawn* FollowingPawn = Pawn->nextPawn;   // [KHG] cache before the events: eventSeePlayer/eventHearNoise can DestroyActor a PawnList pawn, freeing `Pawn`
