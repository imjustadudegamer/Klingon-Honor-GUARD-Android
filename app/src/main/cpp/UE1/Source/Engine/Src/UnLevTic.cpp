	...
	&&	(!NetDriver || !NetDriver->ServerConnection || NetDriver->ServerConnection->State==USOCK_Open) )
	{
		...
		while( NewlySpawned && Updated )
		{
			...
			for( Link; Link; Link=Link->Next )
				if( Link->Actor && !Link->Actor->bDeleteMe )   // [KHG] guard: a newly-spawned actor can be destroyed during the main tick pass; the main loop above already null-checks Actors(iActor)
