...
#include "UnRender.h"
#include "UnNet.h"
#if defined(PLATFORM_ANDROID) // UNREAL_ANDROID_PERF_PROFILE_V1
...
UBOOL UGameEngine::Browse( FURL URL, char* Error256 )
{
	...
	else if( URL.HasOption("pop") )
	{
		...
		if( GLevel && GLevel->GetLevelInfo()->HubStackLevel>0 )
		{
			...
			URL = FURL( &URL, Bare, TRAVEL_Partial );
			URL.Map = Filename;
	...
	else if( (Option=URL.GetOption("load=",NULL))!=NULL )
	{
		...
		LoadFileURL.Map = Temp;
		if( LoadMap(LoadFileURL,NULL,Error256) )
		{
			// Copy the hub stack.
			INT i;
...
static void UE1FMVPresentFrame( UViewport* Viewport, FSceneNode* Frame, UBOOL bBlackBackdrop )
{
	...
	const unsigned char* FMVData = NULL; INT FMVW = 0, FMVH = 0;
	if( !Viewport->RenDev || !UE1FMVGetFrameBGRA( &FMVData, &FMVW, &FMVH ) || !FMVData || FMVW <= 0 || FMVH <= 0 )
	{
		UE1FMVDrawActiveFrame();   // legacy GLES path (no-ops cleanly when no GL context)
		return;
	}

	FMipmap FMVMip;
	FMVMip.DataPtr = (BYTE*)FMVData;
	FMVMip.USize = FMVW; FMVMip.VSize = FMVH;
	FMVMip.UBits = 0; FMVMip.VBits = 0;
	FTextureInfo FMVInfo;
	appMemset( &FMVInfo, 0, sizeof(FMVInfo) );
	FMVInfo.CacheID      = (QWORD)0x4B4847464D560001ULL;  // fixed "KHGFMV\0\1" slot (one reused image)
	FMVInfo.Format       = TEXF_RGB32;   // 32-bit BGRA (VulkanDrv GetTexture swaps to RGBA)
	FMVInfo.UScale       = FMVInfo.VScale = 1.f;
	FMVInfo.USize        = FMVW; FMVInfo.VSize = FMVH;
	FMVInfo.UClamp       = FMVW; FMVInfo.VClamp = FMVH;
	FMVInfo.NumMips      = 1;
	FMVInfo.Mips[0]      = &FMVMip;
	FMVInfo.Palette      = NULL;
	FMVInfo.TextureFlags = TF_RealtimeChanged;   // force re-upload of the new frame each tick
	// Letterbox to the frame, preserving the video's aspect.
	FLOAT Sx = Frame->FX, Sy = Frame->FY;
	...
	{
		FMipmap OvMip;
		OvMip.DataPtr = (BYTE*)OvData;
		OvMip.USize = OvW; OvMip.VSize = OvH; OvMip.UBits = 0; OvMip.VBits = 0;
		FTextureInfo OvInfo;
		appMemset( &OvInfo, 0, sizeof(OvInfo) );
		OvInfo.CacheID      = (QWORD)0x4B4847464D560002ULL;  // distinct "KHGFMV\0\2" overlay slot
		OvInfo.Format       = TEXF_RGB32;
		OvInfo.UScale       = OvInfo.VScale = 1.f;
		OvInfo.USize        = OvW; OvInfo.VSize = OvH;
		OvInfo.UClamp       = OvW; OvInfo.VClamp = OvH;
		OvInfo.NumMips      = 1;
		OvInfo.Mips[0]      = &OvMip;
		OvInfo.Palette      = NULL;
		OvInfo.TextureFlags = TF_RealtimeChanged;   // re-upload (overlay differs per briefing)
		Viewport->RenDev->DrawTile( Frame, OvInfo, Dx, Dy, Dw, Dh, 0.f, 0.f, (FLOAT)OvW, (FLOAT)OvH,
			NULL, 1.f, FPlane(1,1,1,1), FPlane(0,0,0,0), PF_NoSmooth | PF_Masked );
	}
}
#endif
...
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	if( bFMVActive )
		UE1FMVPresentFrame( Viewport, Frame, /*bBlackBackdrop=*/1 );
#endif
...
void UGameEngine::Tick( FLOAT DeltaSeconds )
{
...
#if defined(PLATFORM_ANDROID) // UNREAL_ANDROID_PERF_PROFILE_V1 — CPU frame breakdown to Khg.log (grep KHG_PERF)
	{
		...
		if( kPerfOn )
		{
			static DOUBLE accTick=0.0, accGame=0.0, accClient=0.0; static INT pfN=0;
			accTick   += (DOUBLE)TickCycles   * GSecondsPerCycle * 1000.0;
			accGame   += (DOUBLE)GameCycles   * GSecondsPerCycle * 1000.0;
			accClient += (DOUBLE)ClientCycles * GSecondsPerCycle * 1000.0;
			if( ++pfN >= 120 )
			{
				DOUBLE t = accTick/pfN, g = accGame/pfN, c = accClient/pfN;
				__android_log_print( ANDROID_LOG_INFO, "KHG_PERF",
					"cpu/frame total=%.2fms game=%.2fms render+present=%.2fms | %.0f fps (%s-bound)",
					t, g, c, (t>0.0)?1000.0/t:0.0, (c>g)?"render/gpu":"game" );
				accTick=accGame=accClient=0.0; pfN=0;
			}
		}
	}
#endif
